...
var _sprite_rot_offset: float = 0.0
...
func _setup_sprite() -> void:
	var tex := load("res://assets/2026-03-18-faction-science-vessel.png") as Texture2D
...
func _draw() -> void:
	if _has_sprite and is_instance_valid(_sprite_tex):
		...
		# Debug label
		draw_set_transform(Vector2.ZERO, -rotation, Vector2.ONE)
		draw_string(ThemeDB.fallback_font, Vector2(-20, _sprite_size * 0.5 + 10), "SCIENCE",
			HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(0.4, 0.9, 0.9))
		draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)
		return
	if is_dead:
		return
